...
export const PhotoUpload: React.FC<PhotoUploadProps> = ({ photo, onPhotoChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Hidden File Input for Native OS Integration */}
      <input
        ref={fileInputRef}
        type="file"
        accept="image/*"
      ...
      {photo ? (
        /* VIEW 1: Photo Selected / Uploaded Preview */
          ...
          <div className="w-full h-full rounded-[22px] overflow-hidden relative">
            ...
            <div className="absolute bottom-3 left-3 right-3 flex items-center justify-center">
              <button
                ...
                className="w-full py-2.5 px-4 rounded-xl bg-surface-200/90 hover:bg-surface-100 backdrop-blur-md border border-white/10 text-white text-xs font-semibold flex items-center justify-center gap-2 transition-colors shadow-lg"
              >
        ...
        /* VIEW 2: Empty State - Upload Action */
        <div className="space-y-4">
          <div className="w-full max-w-sm mx-auto aspect-square rounded-3xl border-2 border-dashed border-cosmic-purple/40 bg-surface-100/60 p-6 flex flex-col items-center justify-center text-center space-y-5 shadow-inner transition-colors hover:bg-surface-100/80">
            <div className="w-16 h-16 rounded-full bg-cosmic-purple/15 text-cosmic-purple border border-cosmic-purple/30 flex items-center justify-center shadow-cosmic">
              <ImageIcon className="w-8 h-8 text-amber-300 animate-pulse" />
            </div>

            <div className="space-y-1.5">
              <h3 className="text-base font-bold text-white">Add Your Cosmic Portrait</h3>
              <p className="text-xs text-text-secondary max-w-xs leading-relaxed">
                Upload a clear, authentic photo from your device library to help matches connect with your energy.
              </p>
            </div>
            ...
            <div className="flex flex-col w-full gap-2.5 pt-2">
              <button
                ...
                onClick={() => fileInputRef.current?.click()}
                className="w-full py-3.5 px-4 rounded-2xl bg-gradient-to-r from-cosmic-purple to-cosmic-pink hover:opacity-90 text-white text-sm font-bold shadow-cosmic flex items-center justify-center gap-2 transition-all hover:scale-[1.02]"
              >
